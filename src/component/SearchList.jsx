...
export default function SearchList({pageInfo, searchList, currentPage}) {
    const dispatch = useDispatch()
    const searchParams = useQuery()
    const query = searchParams.get('q')
    const searchState = useSelector((state) => {if (state.search.query=== query) {
        return {result: state.search.result, currentPage: state.search.currentPage, meta: state.search.meta}
    }})
   console.log('searchState', searchState)
    ...
    const render = () => {
        ...
        console.log('result, currentPage, meta', result, currentPage, meta)
        const {loading, error, list} =  result?.[currentPage] || {}
        console.log('loading, error, list', loading, error, list)
        if (error) {
